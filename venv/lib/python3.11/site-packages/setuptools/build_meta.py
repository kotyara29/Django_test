...
Previously, when a customer or a command line tool (let's call it a "frontend")
...
"""
Currently the customer can run::
...
class _ConfigSettingsTranslator:
    ...
    def _global_args(self, config_settings: _ConfigSettings) -> Iterator[str]:
        """
        Let the customer specify ``verbose`` or ``quiet`` + escape hatch via
        ``--global-option``.
        ...
        ['-v']
        >>> list(fn({"verbose": "true", "--global-option": "-q --no-customer-cfg"}))
        ['-v', '-q', '--no-customer-cfg']
        >>> list(fn({"--quiet": None}))
        ['-q']
        """
